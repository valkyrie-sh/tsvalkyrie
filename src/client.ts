...
import { Flake, FlakeRetrieveResponse } from './resources/flake';
import { Health } from './resources/health';
import {
  JobListExecutionsParams,
  JobListExecutionsResponse,
  JobRetrieveJobExecutionsParams,
  JobRetrieveJobExecutionsResponse,
  Jobs,
  Pagination,
} from './resources/jobs';
import {
...
export interface ClientOptions {
  /**
   * Defaults to process.env['TSVALKYRIE_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['TSVALKYRIE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TSVALKYRIE_LOG'] or 'warn' if it isn't set.
   */
...
export class Tsvalkyrie {
   ...
   */
  constructor({
    baseURL = readEnv('TSVALKYRIE_BASE_URL'),
    apiKey = readEnv('TSVALKYRIE_API_KEY') ?? null,
    ...opts
  }: ClientOptions = {}) {
    ...
    this.timeout = options.timeout ?? Tsvalkyrie.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('TSVALKYRIE_LOG'), "process.env['TSVALKYRIE_LOG']", this) ??
      defaultLogLevel;
  ...
  protected stringifyQuery(query: Record<string, unknown>): string {
    return Object.entries(query)
      ...
      .map(([key, value]) => {
        ...
        throw new Errors.TsvalkyrieError(
  ...
  static Tsvalkyrie = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static TsvalkyrieError = Errors.TsvalkyrieError;
  ...
  languages: API.Languages = new API.Languages(this);
  languageVersions: API.LanguageVersions = new API.LanguageVersions(this);
  sandbox: API.Sandbox = new API.Sandbox(this);
  flake: API.Flake = new API.Flake(this);
  executions: API.Executions = new API.Executions(this);
}
Tsvalkyrie.Health = Health;
Tsvalkyrie.Jobs = Jobs;
Tsvalkyrie.Version = Version;
Tsvalkyrie.Languages = Languages;
Tsvalkyrie.LanguageVersions = LanguageVersions;
Tsvalkyrie.Sandbox = Sandbox;
Tsvalkyrie.Flake = Flake;
Tsvalkyrie.Executions = Executions;
export declare namespace Tsvalkyrie {
  ...
  export {
    LanguageVersions as LanguageVersions,
